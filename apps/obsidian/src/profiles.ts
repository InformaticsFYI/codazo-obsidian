import { z } from 'zod';
import type { OpenAICompatibleConfig } from '../../../packages/shared/lib/review/openai-compatible';
import { destinationLabel, isPrivateHost } from '../../../packages/shared/lib/review/openai-compatible';
import type { ReviewSettingsSource } from '../../../packages/shared/lib/review/local-review-service';
import { parseStrictJson } from '../../../packages/shared/lib/security/strict-json';
import { ENDPOINTS, KeySchema } from '../../../packages/shared/lib/settings/provider-settings';
import type { KeyVault, SettingsPersistence } from '../../../packages/shared/lib/settings/local-settings-store';
...
export class ProfileStore implements ReviewSettingsSource {
  ...
  async save(input: unknown, apiKey?: string): Promise<Profile> {
    ...
    if (!parsed.success) throw new Error(parsed.error.issues.some(i => i.message === 'INVALID_BASE_URL') ? (validateCustomURL(String((input as { baseURL?: unknown })?.baseURL ?? '')) ?? 'INVALID_BASE_URL') : 'INVALID_PROFILE');
