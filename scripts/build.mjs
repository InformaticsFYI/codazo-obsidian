 ...
 * styles.css, plus RELEASE.json with their SHA-256 and a regenerated NOTICE.md. Browser platform, no Node built-ins, Obsidian and CodeMirror
...
const result = await build({
  absWorkingDir: root, entryPoints: ['src/main.ts'], outfile: join(out, 'main.js'), bundle: true, format: 'cjs', platform: 'browser', target: 'es2022',
...
for (const [path, input] of Object.entries(result.metafile.inputs)) {
  if (!/^(src\/|shared\/|node_modules\/|marker:)/.test(path)) throw new Error(`Unexpected bundle input: ${path}`);
  const key = path.startsWith('node_modules/') ? path.slice(13).split('/').slice(0, path.startsWith('node_modules/@') ? 2 : 1).join('/') : path.startsWith('shared/') ? 'shared' : 'src';
...
await writeFile(join(out, 'meta.json'), JSON.stringify(result.metafile, null, 2));
...
await writeFile(join(out, 'RELEASE.json'), JSON.stringify({ version: JSON.parse(await readFile(join(app, 'manifest.json'), 'utf8')).version, hashes }, null, 2) + '\n');
