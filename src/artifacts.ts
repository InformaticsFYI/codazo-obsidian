import { validateReview } from '../shared/lib/review/validate';
...
export function parseSavedReview(content: string): { ok: true; value: SavedReview } | { ok: false; error: string } {
  ...
  const property = (key: string): string | null => { const match = content.match(new RegExp(`^${key}: (.*)$`, 'm')); if (!match) return null; try { const value: unknown = JSON.parse(match[1]); return typeof value === 'string' ? value : null; } catch { return null; } };
