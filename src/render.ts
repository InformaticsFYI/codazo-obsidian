...
import { resolveAnchor } from '../shared/lib/review/resolve-annotations';
import type { Annotation, Review } from '../shared/lib/review/schema';
...
export function el<K extends keyof HTMLElementTagNameMap>(parent: Node, tag: K, options: { text?: string; cls?: string; lang?: string; attrs?: Record<string, string> } = {}): HTMLElementTagNameMap[K] {
  // Obsidian's createEl creates the node in the parent's own window (popout-safe) and sets text through textContent; tests polyfill it under jsdom.
  const node = parent.createEl(tag, { ...(options.cls ? { cls: options.cls } : {}), ...(options.text !== undefined ? { text: options.text } : {}), ...(options.attrs ? { attr: options.attrs } : {}) });
